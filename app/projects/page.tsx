      ...
      <div className="container py-12">
        <ul className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <li key={project.slug} className="group overflow-hidden rounded-lg border border-gray-200 dark:border-gray-800">
              {project.image ? (
                <Link href={`/projects/${project.slug}`} aria-label={`Open ${project.title}`}>
                  <Image src={project.image} alt={project.title} width={1200} height={630} className="h-40 w-full object-cover transition-transform hover:scale-[1.01]" />
                </Link>
              ) : null}
              <div className="p-5">
                ...
                {project.mentors && <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">Mentor: {project.mentors}</p>}
                <p className="mt-1 text-xs text-gray-400">
                  {format(new Date(project.date), 'MMM yyyy')}
                  {project.endDate ? ` – ${format(new Date(project.endDate), 'MMM yyyy')}` : ''}
                </p>
                {project.excerpt && <p className="mt-3 text-sm text-gray-600 dark:text-gray-300">{project.excerpt}</p>}
