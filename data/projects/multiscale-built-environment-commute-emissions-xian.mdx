...
For commuter _i_ using mode _n_:

**WCE<sub>i</sub> = D<sub>i</sub> × F<sub>n</sub> × C<sub>i</sub>**

where D<sub>i</sub> is residence–workplace distance, F<sub>n</sub> is the mode-specific emission factor, and C<sub>i</sub> is a traffic-condition correction determined by urban zones. Non-motorized modes are set to zero.

#### Table A. Mode-specific CO₂ emission factors (used in F<sub>n</sub>)
...
#### Table B. Traffic-condition correction factors by urban zone (used in C<sub>i</sub>)
...
**LUM<sub>i</sub> = -(Σ<sub>a</sub> p<sub>a</sub> ln p<sub>a</sub>) / ln N**

where p<sub>a</sub> is the share of land-use class _a_ in region _i_.

- **Job accessibility index (JAI)** at Jiedao scale:

**JAI<sub>i</sub> = (W<sub>i</sub> + Σ<sub>j=1..S</sub> W<sub>j</sub>/d<sub>ij</sub><sup>2</sup>) / WP<sub>i</sub>**

where W is job supply, WP is working-age population, and d<sub>ij</sub> are centroid distances within 4 km.
...
Let commuter _i_ live in residence community _r_ and work in workplace community _w_.

**Level-1 (individual):**

**WCE<sub>irw</sub> = φ + α<sub>r</sub> + α<sub>w</sub> + β<sub>S</sub><sup>T</sup> X<sub>i</sub> + e<sub>irw</sub>,  where e<sub>irw</sub> ~ N(0, σ<sub>irw</sub><sup>2</sup>)**

**Level-2 (residence):**

**α<sub>r</sub> = β<sub>R</sub><sup>T</sup> X<sub>r</sub> + e<sub>r</sub>,  where e<sub>r</sub> ~ N(0, σ<sub>r</sub><sup>2</sup>)**

**Level-2 (workplace):**

**α<sub>w</sub> = β<sub>W</sub><sup>T</sup> X<sub>w</sub> + e<sub>w</sub>,  where e<sub>w</sub> ~ N(0, σ<sub>w</sub><sup>2</sup>)**

**Variance attribution (ICCs):**

**ICC<sub>res</sub> = σ<sub>r</sub><sup>2</sup> / (σ<sub>irw</sub><sup>2</sup> + σ<sub>r</sub><sup>2</sup> + σ<sub>w</sub><sup>2</sup>)**

**ICC<sub>work</sub> = σ<sub>w</sub><sup>2</sup> / (σ<sub>irw</sub><sup>2</sup> + σ<sub>r</sub><sup>2</sup> + σ<sub>w</sub><sup>2</sup>)**
...
![Lorenz curve of weighted commuting CO₂ (67–10 pattern)](/images/projects/commute-carbon/Lorenzcurv.png)
...
Workplace context dominates in the full sample: ICC<sub>work</sub> = 0.731 vs. ICC<sub>res</sub> = 0.078. The combined (res + work) ICC is higher for MHI (0.786) than LI (0.416), implying BE explains more for MHI commuters.

#### Table C. Empty-model variance components and ICCs

| Group | σ<sub>r</sub><sup>2</sup> | σ<sub>w</sub><sup>2</sup> | σ<sub>irw</sub><sup>2</sup> | ICC<sub>res</sub> | ICC<sub>work</sub> | N |
|-------|--------------|--------------|------------------|-------------|--------------|---|
...
|------------------|------------------------|------------------|
| d<sub>center_r</sub> × AHI | positive | < 0.05 |
| num<sub>bus_w</sub> × AHI | negative | < 0.05 |
| metro<sub>w</sub> × AHI | negative | 0.10 to 0.001 |
| den<sub>park_w</sub> × AHI | positive | * to ** |
| ja<sub>w</sub> × AHI | positive | * to *** |
